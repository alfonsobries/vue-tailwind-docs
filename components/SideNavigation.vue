<template>
  <nav>
    <div class="lg:sticky lg:top-0 lg:max-h-screen lg:overflow-y-auto lg:scrolling-touch lg:pt-10 ">
      ...
      >
        <icon class="w-4 h-4 text-blueGray-500">
      ...
      <div v-for="group in docsMenu" :key="group.id" class="my-5">
        <h5 class="px-3 mb-3 text-xs font-bold tracking-wide uppercase text-blueGray-500">
          {{ group.text }}
        </h5>
        <ul>
          <li v-for="(item, index) in group.items" :key="index">
            ...
            >
              {{ item.text }}
              <span v-if="item.soon" class="px-2 py-1 ml-2 text-xs leading-none text-blue-700 bg-blue-100 rounded-lg">
                WIP
              </span>
              <span v-else-if="recentlyUpdated(item)" class="px-2 py-1 ml-2 text-xs leading-none text-blue-700 bg-blue-100 rounded-lg">
                Updated
              </span>
              <span v-else-if="item.since" class="px-2 py-1 ml-2 text-xs leading-none text-blue-700 bg-blue-100 rounded-lg">
                v{{ item.since }}+
              </span>
            </nuxt-link>
          </li>
